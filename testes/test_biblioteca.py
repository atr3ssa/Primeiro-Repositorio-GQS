...
class TestCenariosIntegracao:
    """Testes de integração com cenários complexos do mundo real."""
    
    def test_fluxo_completo_biblioteca(self, biblioteca_nova):
        """
        Testa o fluxo completo de uma biblioteca:
        1. Adicionar livros
        2. Cadastrar usuários
        3. Realizar empréstimos
        4. Devolver livros
        5. Emprestar novamente
        """
        # Fase 1: Setup
        biblioteca_nova.adicionar_livro("Python Fluente", "Luciano Ramalho", "9788575225400", 2015)
        biblioteca_nova.cadastrar_usuario("Ana Costa", "ana@email.com", "11987654321")
        
        assert len(biblioteca_nova.livros) == 1
        assert len(biblioteca_nova.usuarios) == 1
        
        # Fase 2: Empréstimo
        resultado_emp = biblioteca_nova.realizar_emprestimo(usuario_id=1, livro_id=1)
        assert resultado_emp is True
        assert biblioteca_nova.livros[0].disponivel is False
        
        # Fase 3: Devolução
        resultado_dev = biblioteca_nova.devolver_livro(emprestimo_id=1)
        assert resultado_dev is True
        assert biblioteca_nova.livros[0].disponivel is True
        
        # Fase 4: Novo empréstimo
        biblioteca_nova.cadastrar_usuario("Pedro Silva", "pedro@email.com", "11976543210")
        resultado_emp2 = biblioteca_nova.realizar_emprestimo(usuario_id=2, livro_id=1)
        assert resultado_emp2 is True
    
    def test_multiplos_emprestimos_e_devolucoes_parciais(self, biblioteca_com_dados):
        """
        Cenário: Múltiplos usuários pegam vários livros e devolvem parcialmente.
        """
        # Usuário 1 pega 2 livros
        biblioteca_com_dados.realizar_emprestimo(usuario_id=1, livro_id=1)
        biblioteca_com_dados.realizar_emprestimo(usuario_id=1, livro_id=2)
        
        # Usuário 2 pega 1 livro
        biblioteca_com_dados.realizar_emprestimo(usuario_id=2, livro_id=3)
        
        assert len(biblioteca_com_dados.emprestimos) == 3
        assert all(not livro.disponivel for livro in biblioteca_com_dados.livros)
        
        # Usuário 1 devolve apenas 1 livro
        biblioteca_com_dados.devolver_livro(emprestimo_id=1)
        
        assert biblioteca_com_dados.livros[0].disponivel is True
   ...
   ## def test_persistencia_completa_com_reload(self, biblioteca_nova):
       ...
       # biblioteca_nova.devolver_livro(emprestimo_id=1)
        ...
        # Validações
        assert len(nova.livros) == 2
        assert len(nova.usuarios) == 2
        assert len(nova.emprestimos) == 2
        assert nova.emprestimos[0].devolvido is True
        assert nova.emprestimos[1].devolvido is False
        assert nova.livros[0].disponivel is True
        assert nova.livros[1].disponivel is False
